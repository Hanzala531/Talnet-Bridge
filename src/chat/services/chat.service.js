...
import { assertCanStartConversation } from "./roleRules.js";
...
export async function appendMessage({ conversationId, senderId, text, attachments = [], replyTo }) {
  ...
  try {
    ...
    // Sanitize text
    const sanitizedText = text ? Message.sanitizeText(text) : "";
...
export async function markRead({ conversationId, userId, upToMessageId }) {
  ...
  try {
    ...
    if (upToMessageId) {
      ...
      const upToMessage = await Message.findById(upToMessageId).session(session);
      if (upToMessage) {
